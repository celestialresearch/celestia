...
ordinary recovery. If the attempt has recoverable evidence but no v0 ownership
lock, `Recover` returns `ErrMigrationRequired`. An operator or migration tool
...
validation and publication path. A missing lock is therefore an explicit
version boundary, not a stale-lock or sidecar-recovery signal.
